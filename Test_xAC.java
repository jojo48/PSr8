...
//    public String sub_field_xAC_30(int[] raw_field_xAC_30) {
        ...
        if (raw_xAC_30[0] == 0x30) {  // Check sub_tag level 1 (x30)
            xAC_indx++;
            int xAC_30_len = raw_xAC_30[xAC_indx];  // may be not use
            xAC_indx++;
            
            do {
                int raw_xAC_30_sub1_len = raw_xAC_30[xAC_indx + 1];
                int[] sub_raw_xAC_30 = new int[raw_xAC_30_sub1_len + 2];

                for (int i = 0; i < (raw_xAC_30_sub1_len + 2); i++) {
                    sub_raw_xAC_30[i] = raw_xAC_30[xAC_indx];
                    xAC_indx++;
                }
                System.out.println("Dump array sub xAC30:" + DataConverter.Int2HexString(sub_raw_xAC_30));    //Debug
                decode_data_xAC_30_sub1 = SubFieldDecoder.decode_xAC_x30(sub_raw_xAC_30);
